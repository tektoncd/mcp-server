...
func TestHandlerListStepActions(t *testing.T) {
	...
	tests := []struct {
		name          string
		request       *mcp2.CallToolParamsFor[listParams]
		expectedNames []string
	}{
		{
			name:          "No namespace, no filters",
			request:       &mcp2.CallToolParamsFor[listParams]{Arguments: listParams{}},
			expectedNames: []string{"foo-sa1", "bar-sa2"},
		},
		{
			name:          "With namespace",
			request:       &mcp2.CallToolParamsFor[listParams]{Arguments: listParams{Namespace: "ns1"}},
			expectedNames: []string{"foo-sa1"},
		},
		{
			name:          "With label selector",
			request:       &mcp2.CallToolParamsFor[listParams]{Arguments: listParams{LabelSelector: "env=dev"}},
			expectedNames: []string{"bar-sa2"},
		},
		{
			name:          "With prefix filter",
			request:       &mcp2.CallToolParamsFor[listParams]{Arguments: listParams{Prefix: "foo"}},
			expectedNames: []string{"foo-sa1"},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			actual := res.Content[0].(*mcp2.TextContent)
