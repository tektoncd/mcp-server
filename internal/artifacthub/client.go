...
func (c *Client) SearchPackages(ctx context.Context, opts SearchOptions) (*SearchResponse, error) {
	endpoint := fmt.Sprintf("%s/packages/search", c.baseURL)
...
func (c *Client) GetPackageContent(ctx context.Context, contentURL string) (string, error) {
	if contentURL == "" {
		return "", fmt.Errorf("content URL is empty")
	}
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return "", fmt.Errorf("content request failed with status %d: %s", resp.StatusCode, string(body))
	}
